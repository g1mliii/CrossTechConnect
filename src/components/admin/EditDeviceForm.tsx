/**
 * Edit Device Form - Interface for editing existing devices
 */
...
export function EditDeviceForm({ deviceId }: EditDeviceFormProps) {
    ...
    try {
      ...
      if (data.success) {
        ...
        setFormData({
          ...
          manualUrl: device.manual_url || '',
          description: device.description || '',
          verified: device.verified || false,
          extractionMethod: device.extraction_method || 'manual'
        });

        // Fetch device specifications if they exist
        if (device.category_id) {
          fetchDeviceSpecifications();
        }
      } else {
        setValidationErrors(['Failed to load device']);
      }
    } catch (error) {
  ...
  const fetchDeviceSpecifications = async () => {
    try {
      ...
      // Specifications might not exist yet, that's okay
      console.log('No specifications found for device');
    }
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (deviceResponse.ok) {
        ...
        setValidationErrors([error.error || 'Failed to update device']);
      }
    } catch (error) {
      console.error('Failed to update device:', error);
      setValidationErrors(['Failed to update device']);
    } finally {
      setSaving(false);
    }
  };
